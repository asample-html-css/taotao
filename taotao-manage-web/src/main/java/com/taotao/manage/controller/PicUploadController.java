...
/**
 * 图片上传  将图片上传，返回PicUploadResult，内含图片地址 长宽等信息
 */
@Controller
@RequestMapping("pic")
public class PicUploadController {
	...
	public String upload(@RequestParam("uploadFile") MultipartFile uploadFile , HttpServletResponse response) throws Exception {
		...
		// 生成图片的绝对引用地址  访问http://image.taotao.com将对应到本地地址D:\taotao-upload
		String picUrl =
				StringUtils.replace(StringUtils.substringAfter(filePath, propertiesService.REPOSITORY_PATH), "\\", "/");
		...
		return mapper.writeValueAsString(fileUploadResult);//将返回PicUploadResul序列化成json数据
	}
